...
| ---------------------- | ------------------------------------------------------------------------------------------------ |
| ID                     | UC-02                                                                                            |
| Краткое описание       | Пользователь выбирает вид спорта на главной                                                      |
| Главные актёры         | Посетитель                                                                                       |
| Второстепенные актёры  | Нет                                                                                              |
| Предусловия            | Открыта главная страница                                                                         |
| Основной поток         | Пользователь выбирает вкладку спорта (футбол/теннис/хоккей/MMA); система обновляет список матчей |
| Постусловия            | Показаны матчи выбранного спорта                                                                 |


### Прецедент UC-03


| Прецедент: FilterByDateOnHome |                                                                                       |
| ----------------------------- | ------------------------------------------------------------------------------------- |
| ID                            | UC-03                                                                                 |
| Краткое описание              | Пользователь меняет фильтр даты на главной                                            |
| Главные актёры                | Посетитель                                                                            |
| Второстепенные актёры         | Нет                                                                                   |
| Предусловия                   | Открыта главная страница с тегами даты                                                |
| Основной поток                | Пользователь нажимает тег даты (`data-qa="Tag"`); система перестраивает выдачу матчей |
| Постусловия                   | Список матчей соответствует выбранной дате                                            |
...
| Прецедент: OpenMatchCenter |                                                                            |
| -------------------------- | -------------------------------------------------------------------------- |
| ID                         | UC-08                                                                      |
| Краткое описание           | Пользователь открывает матч-центр                                          |
| Главные актёры             | Посетитель                                                                 |
| Второстепенные актёры      | Нет                                                                        |
| Предусловия                | Доступен сайт                                                              |
| Основной поток             | Пользователь переходит на `/stat/football/`; система отображает матч-центр |
| Постусловия                | Доступен список матчей по футболу                                          |


### Прецедент UC-09


| Прецедент: SwitchDayInMatchCenter |                                                                                  |
| --------------------------------- | -------------------------------------------------------------------------------- |
| ID                                | UC-09                                                                            |
| Краткое описание                  | Пользователь переключает день в матч-центре                                      |
| Главные актёры                    | Посетитель                                                                       |
| Второстепенные актёры             | Нет                                                                              |
| Предусловия                       | Открыт матч-центр                                                                |
| Основной поток                    | Пользователь выбирает «Вчера/Сегодня/Завтра»; система обновляет календарь матчей |
| Постусловия                       | Показан список матчей выбранного дня                                             |
...
| 2 (UC-02)  | Главная страница загружена              | Клик по вкладке вида спорта (например, «теннис»)              | Фильтрация матчей по выбранному виду спорта | Список матчей/контент обновлён                    | Главная в режиме выбранного спорта |
| 3 (UC-03)  | Главная в режиме спорта `/stat/futbol`  | Клик по дате (напр. 2-й/3-й)                                  | Переключение периода отображения            | Изменился набор матчей/состояние тега             | Главная в режиме выбранной даты    |
...
| 8 (UC-08)  | Вне матч-центра                         | Открываем `/stat/football/`                                   | Загрузка раздела матч-центра                | Заголовок и список матчей по футболу              | Матч-центр открыт                  |
| 9 (UC-09)  | Матч-центр открыт                       | Клик по «Завтра»                                              | Переключение дня в календаре матчей         | Обновился список/заголовок периода                | Матч-центр в режиме «Завтра»       |
| 10 (UC-10) | Вне страницы турнира                    | Открываем `/stat/futbol/tournament/667/`                      | Загрузка страницы турнира                   | Видны календарь/таблица/описание турнира          | Страница турнира открыта           |
| 11 (UC-11) | Страница матча открыта                  | Проверка блоков статистики                                    | Система отображает секции матча             | Видны «Личные встречи»/«Состав команды»/«События» | Статистика матча доступна          |
| 12 (UC-12) | Вне раздела БК                          | Открываем `/bukmekery/luchshie/`                              | Загрузка рейтинга букмекеров                | В тексте присутствуют FONBET/Winline и др.        | Рейтинг БК открыт                  |
| 13 (UC-13) | Вне обзора букмекера                    | Открываем `/fonbet/` (или другой БК)                          | Загрузка страницы обзора БК                 | Видны название, рейтинг, описание, CTA            | Обзор букмекера открыт             |
| 14 (UC-14) | Вне раздела бонусов                     | Открываем `/bukmekery/luchshie-fribety/`                      | Загрузка витрины бонусов                    | Видны фрибеты/суммы/карточки БК                   | Раздел бонусов открыт              |
| 15 (UC-15) | Главная или бонусы открыты              | Клик «Получить фрибет»                                        | Выполняется партнёрский редирект            | Открыт внешний URL/новая вкладка                  | Переход за фрибетом выполнен       |
| 16 (UC-16) | Вне раздела новостей                    | Открываем `/mag/novosti/` и кликаем новость                   | Переход на страницу новости                 | URL вида `/mag/novosti/...`, текст статьи         | Новость открыта                    |
| 17 (UC-17) | Вне раздела прогнозов                   | Открываем `/prognozy/football/`, открываем первую статью      | Переход на страницу прогноза                | URL `/prognozy/...` и контент статьи              | Прогноз открыт                     |
| 18 (UC-18) | Энциклопедия не открыта                 | Открываем `/enciklopediya/`, вводим «тотал», нажимаем «Найти» | Выполняется поиск по энциклопедии           | Показаны результаты по запросу                    | Результаты поиска отображены       |
| 19 (UC-19) | В результатах энциклопедии / прямой URL | Открываем статью `.../total-4-5-bolshe...`                    | Загрузка страницы статьи                    | Виден заголовок и основной текст                  | Статья энциклопедии открыта        |
